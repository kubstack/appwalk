...
model: ${MODEL}
maxSteps: 25
# Explore this many runs at once instead of one after another. Defaults to 1.
# maxConcurrentPersonas: 2
# browser: firefox  # defaults to chromium; webkit is also available
# trace: true  # save a Playwright trace archive for exploration and each replayed flow
screenshots: true
...
safety:
  allowDestructive: false # set true only against a disposable or staging target where mutations are safe
...
| `persona`                   | string               | No                           | Built-in persona for a single run. Use `runs` for multiple independent personas.                                                                                                                                                                                                                  |
| `maxSteps`                  | positive integer     | No                           | Default action budget per run is 25.                                                                                                                                                                                                                                                              |
| `maxConcurrentPersonas`     | positive integer     | No                           | How many `runs` entries explore at once. Defaults to `1` (sequential). See [Multi-person coverage](#multi-person-coverage).                                                                                                                                                                       |
...
| `runs`                      | run list             | No                           | Independent persona runs. Every run needs a `name`; `persona`, `maxSteps`, `scope`, `expect`, `email`, `password`, and `storageState` override global values for that run. A run's `email`/`password`/`storageState` fully replaces the global `auth.*` for that run rather than merging with it. |

## Response scenarios

Response scenarios (`responses.maxVariants`, `responses.maxFixtureBytes`) are optional derived flows, independent of which persona discovered the base flow. They apply whenever a verified flow observes a meaningful JSON response with business-state fields such as order status, inventory, totals, or a collection. The CLI flag and config field call this a "variant" (`--response-variant-max`, `responses.maxVariants`); "scenario" and "variant" refer to the same thing throughout this documentation.
