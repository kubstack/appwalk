...
| `--max-concurrent-personas <number>`    | `run`, `explore`             | Never                                                                               | How many `runs` entries explore at once, each with its own browser. Defaults to `1` (sequential, the historical behavior); must be a positive integer. Requests across concurrent personas still share the account's real rate limit; see [Multi-person coverage](configuration.md#multi-person-coverage). |
...
Color is a visual aid only in interactive terminals; the message itself always carries the meaning.
Colors are disabled when output is redirected or `NO_COLOR` is set, and `FORCE_COLOR=1` enables
them explicitly. Debug records keep their stable
...
configuration. With `runs`, each persona run is listed separately with its
