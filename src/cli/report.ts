...
function reportSteps(
  ...
  errorLabel: string,
  redactor: ExplorationBatch['redactor'],
): ReportStep[] {
  return entries
...
function reportFlowsForRun(run: ExplorationRun, redactor: ExplorationBatch['redactor']): ReportFlow[] {
  const discovered = (run.discovery?.flows ?? []).map((flow, index): ReportFlow => {
    ...
    const runtimeIssues = run.runtimeErrors.filter(
      (error) => error.phase === 'replay' && error.flowIndex === index + 1 && !error.lifecycle,
    );
    return {
  ...
  const derived = run.confirmedFlows
    ...
    .map((flow, index): ReportFlow => ({
      ...
      ...(flow.sourceFlowIndex !== undefined ? { parentFlowId: `${run.runId}-flow-${flow.sourceFlowIndex + 1}` } : {}),
