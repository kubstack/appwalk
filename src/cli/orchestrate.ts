...
import type { ReportFlow, ReportResponseVariantAudit, ReportRuntimeError, ReportSafety } from '../report/contract.js';
import { extractActions } from '../verify/replay.js';
...
import { closeBrowserWithTimeout, closeTrackedContexts, createBrowserLifecycle } from './browser-lifecycle.js';
import {
...
export { runOutcome, summarizeSafety, hasMinimumChallengeEvidence } from './run-summaries.js';
export { writeDiscoveryArtifacts } from './discovery-manifest-writer.js';
...
async function navigateOrLogin(
  page: Page,
  args: CliArgs,
...
async function exploreAndVerifyInBrowser(
  ...
  try {
    ...
    await runLogger.task('Navigating to application URL', () =>
      navigateOrLogin(page, args, false, args.url, runLogger),
    );
    ...
    if (explorationBlockedRequests > 0) {
      runLogger.warn(
        `Safety policy blocked ${explorationBlockedRequests} destructive request${explorationBlockedRequests === 1 ? '' : 's'} during exploration`,
      );
    }
    runLogger.info(
      `Exploration completed: ${discovery.flows.length} flow(s) found${discovery.exhausted ? '; action budget reached' : ''}`,
    );
    runLogger.debug('exploration.completed', 'Exploration completed', {
    ...
    const runtimeErrorEntries: Array<{
      error: RuntimeErrorEntry;
      phase: 'exploration' | 'replay';
      flowIndex?: number;
    }> = recorder.runtimeErrors.map((error) => ({ error, phase: 'exploration' }));
    let runError: string | undefined;

    if (verifiedFlows.length > 0) {
      runLogger.phase(
        `Verifying ${verifiedFlows.length} of ${discovery.flows.length} discovered flow(s) by replay in a clean session`,
      );
      ...
      try {
        ...
        try {
          for (const { flow, index } of verifiedFlows) {
            ...
            const hasCoreAction =
              !persona?.coreActionTypes || actions.some((action) => persona.coreActionTypes!.includes(action.name));
            ...
            runtimeErrorEntries.push(
              ...replayRecorder.runtimeErrors.map((error) => ({
                error,
            ...
            if (findingCandidate) {
              ...
              flowLogger.warn(
                `Flow ${index + 1}: ${confirmedFinding ? 'finding confirmed' : 'finding inconclusive'}${finding.failure ? ` (${finding.failure})` : ''}`,
              );
              continue;
            }
            ...
            if (!replayResult.reproduced) {
              if (replayResult.failedAt) {
                flowLogger.warn(
                  `Flow ${index + 1} not confirmed: replay could not complete the ${replayResult.failedAt.action} action at step ${replayResult.failedAt.index + 1}`,
                );
                flowLogger.debug('replay.step_failed', 'Replay action failed', {
                  stepIndex: replayResult.failedAt.index,
                  action: replayResult.failedAt.action,
                  error: replayResult.failedAt.error,
                });
                ...
                flowLogger.warn(
                  `Flow ${index + 1} not confirmed: replay was limited by safety policy (${replayResult.safetyBlocked} request${replayResult.safetyBlocked === 1 ? '' : 's'} blocked)`,
                );
              } else {
            ...
            const responseVariantAudit: ReportResponseVariantAudit = {
              ...
              planningStatus:
                (args.responseVariantMax ?? 0) > 0 && (baseFlow.responseFixtures?.length ?? 0) > 0
                  ? 'completed'
                  : (args.responseVariantMax ?? 0) > 0
                    ? 'not_run'
                    : 'not_enabled',
              plannerCandidates: 0,
              plannerRejected: 0,
          ...
          if (replayBlockedRequests > 0) {
            runLogger.warn(
              `Safety policy blocked ${replayBlockedRequests} destructive request${replayBlockedRequests === 1 ? '' : 's'} during replay`,
            );
          }
...
export async function exploreRuns(args: CliArgs, executionId: string, signal?: AbortSignal): Promise<ExplorationBatch> {
  ...
  const runs = await runWithConcurrencyLimit(configuredRuns, concurrency, async (configuredRun, runIndex) => {
    ...
    try {
      ...
      if (runOutcome(run).stopReason !== 'completed') {
        const reason =
          runOutcome(run).stopReason === 'budget_exhausted'
  ...
  if (evidence.issues.length > 0) {
    appLogger.warn(
      `Evidence warning: skipped ${evidence.issues.length} malformed record${evidence.issues.length === 1 ? '' : 's'}`,
    );
  ...
  const confirmedFlows = runs.flatMap((run) =>
    run.confirmedFlows.map((flow) => ({
      ...flow,
