...
export function parseArgs(argv: string[]): CliArgs {
  ...
  const flagKeys = new Map([
    ...
    ['--block-methods', 'blockMethods'],
    ['--safety-config', 'safetyConfigPath'],
  ...
  for (let i = 0; i < rest.length; i += 1) {
    ...
    if (flagKey === undefined) {
      return printUsage(
        flag.startsWith('-') ? `Unknown option "${flag}".` : `Unexpected positional argument "${flag}".`,
      );
    }
...
export function applyConfig(args: CliArgs): CliArgs {
  if (!args.configPath) {
    ...
    if (args.expectations.length > 0 && !args.scope) {
      return printUsage(
        '--expect requires --scope. Expectations describe what should hold within a scoped exploration.',
      );
    }
    return args;
  }
  ...
  if (!args.cliSpecified.has('storageStatePath') && config.auth?.storageState)
    args.storageStatePath = config.auth.storageState;
  if (!args.cliSpecified.has('allowDestructive') && config.safety?.allowDestructive !== undefined)
    args.allowDestructive = config.safety.allowDestructive;
  if (!args.cliSpecified.has('blockMethods') && config.safety?.blockMethods)
    args.blockMethods = normalizeBlockMethods(config.safety.blockMethods);
  ...
  if (!args.provider)
    return printUsage('Missing provider. Pass --provider or define provider in ' + args.configPath + '.');
