...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
    ...
    inputSchema: {
      ...
      properties: {
        url: {
          ...
          minLength: 1,
          maxLength: 8_000,
          description: 'Absolute http or https URL; cross-origin navigation is allowed.',
        },
      },
      required: ['url'],
    },
  ...
  },
  {
    name: 'select',
    description:
      'Choose one option in a <select> dropdown by its value, or multiple options by passing an array of values.',
    inputSchema: {
      type: 'object',
      properties: {
        ...locatorProp,
    ...
    inputSchema: {
      ...
      properties: {
        name: {
          ...
          minLength: 1,
          maxLength: 256,
          description: 'Cookie name to delete; omit to delete all cookies.',
        },
      },
    },
  },
  ...
  },
  {
    name: 'openInNewTab',
    description:
      "Open the current page's URL in a new browser tab and switch to it. The old tab stays open in the background.",
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'openTab',
  ...
  },
  {
    name: 'reopenBrowser',
    description:
      'Simulate fully closing and reopening the browser, then navigate back to the current URL. Cookies and localStorage carry over; sessionStorage does not, matching a real browser restart.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'scroll',
  ...
  },
  {
    ...
    description:
      'Set the browser viewport to a specific width and height in pixels, such as 375 by 667 for a mobile device.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'uploadFile',
    description:
      "Upload one or more approved agent-run input files from the project's agent-inputs/uploads directory. Use relative paths such as agent-inputs/uploads/uma/valid.png; absolute, arbitrary, and outside-root paths are rejected.",
    inputSchema: {
      type: 'object',
      properties: {
        ...locatorProp,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        count: {
          type: 'integer',
          minimum: BURST_MIN_COUNT,
          maximum: BURST_MAX_COUNT,
          description: 'How many times to repeat, e.g. 5. Each repetition consumes one action-budget unit.',
        },
        key: {
          type: 'string',
          minLength: 1,
          maxLength: 100,
          description: 'Required when action is pressKey — the key to press each time.',
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        urlPattern: {
          ...
          minLength: 1,
          maxLength: 2_000,
          description: "A glob pattern matching the request URL, e.g. '**/api/order' or '**checkout**'.",
        },
    ...
    inputSchema: {
      ...
      properties: {
        urlPattern: {
          ...
          minLength: 1,
          maxLength: 2_000,
          description: "A glob pattern matching the request URL, e.g. '**/api/**' or '**checkout**'.",
        },
        delayMs: {
          type: 'number',
          minimum: 0,
          maximum: 60_000,
          description: 'How long to delay the matching request in milliseconds, typically 2000-5000.',
        },
      },
  ...
  },
  {
    ...
    description:
      'Check one expectation against a concrete signal caused by the current flow. When the system prompt supplied a numbered list of expectations for this scope, the index refers to that list. When it did not, there is nothing to reference — use index 1 and choose the check yourself, based on what would actually demonstrate that the flow you just ran did what it was meant to do. Only use this after the current flow has performed the behavior being checked; a read-only page or matching heading from an existing record does not prove that a create, submit, update, complete, or confirm operation happened. Translate the requirement into one observable condition: a locator is visible/hidden, contains exact text, has an exact input value, is checked/unchecked, is enabled/disabled, has an exact count, or the URL contains/equals a value. For a URL check, use urlEquals only for a stable, canonical URL with no run-specific content; if the URL carries a timestamp, session token, search parameters, or any other value that will differ on a later run, use urlContains with a short substring built only from the stable part (e.g. the path), never the full URL — this assertion is replayed as regression coverage long after this run ends, and a value that is only true right now will make every later run fail even though nothing is actually broken. The same caution applies to the locator itself: an id tied to one specific record from live, frequently-changing data will not resolve the same way on a later run, even one moments later — prefer a positional/structural locator for that content instead. When the expectation is that something did NOT go through (rejected, blocked, prevented, removed), assert the absence of the success signal (hidden/unchecked, or count 0) or the presence of an explicit error/validation signal — asserting that the success signal is visible instead would confirm the opposite of what the expectation claims, even though the check itself "passes". Use unknown only when the current flow reached the relevant behavior but no reliable signal can be identified; do not claim a result in plain text.',
    inputSchema: {
      type: 'object',
      properties: {
        expectationIndex: {
          ...
          minimum: 1,
          maximum: 100,
          description: '1-based expectation number from the system prompt.',
        },
        assertion: {
          type: 'string',
          enum: [
        ...
        value: {
          type: 'string',
          description:
            'Expected text, URL, or input value; required for containsText, urlContains, urlEquals, and value.',
        },
        expectedCount: {
          type: 'integer',
          minimum: 0,
          maximum: 10_000,
          description: 'Expected locator count; required for count.',
        },
      },
  ...
  {
    name: 'flowComplete',
    description: 'Call this when you have completed a full, meaningful flow — instead of responding with plain text.',
    inputSchema: {
      type: 'object',
      properties: {
        title: {
          ...
          maxLength: 200,
          description:
            'A short stable title for the behavior, ideally 3-8 words. Omit persona names, concrete data, and IDs.',
        },
        summary: {
          type: 'string',
          description:
