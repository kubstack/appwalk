...
test('maps a global YAML persona into the resolved run options', () => {
  ...
  try {
    writeFileSync(
      path,
      ['version: 1', 'url: https://example.test', 'provider: openai', 'model: test-model', 'persona: noah'].join('\n'),
    );
...
test('rejects missing provider and model instead of applying hidden defaults', () => {
  assert.throws(() => validateResolvedOptions({ ...validOptions, provider: undefined }), /provider must be one of/);
  assert.throws(() => validateResolvedOptions({ ...validOptions, model: '' }), /model must be a non-empty string/);
});
...
test('rejects invalid shared option values', () => {
  ...
  assert.throws(
    () => validateResolvedOptions({ ...validOptions, screenshots: 'yes' }),
    /screenshots must be a boolean/,
  );
  assert.throws(
    () => validateResolvedOptions({ ...validOptions, blockMethods: ['DELETE', ''] }),
    /blockMethods must be a list/,
  );
  assert.throws(
    () => validateResolvedOptions({ ...validOptions, browserEngine: 'safari' }),
    /browser must be one of chromium, firefox, or webkit/,
  );
});
...
test('rejects malformed and non-web target URLs', () => {
  ...
  assert.doesNotThrow(() =>
    validateResolvedOptions({ ...validOptions, url: 'https://login.example.test/oauth/start' }),
  );
});

test('rejects unknown, extra, and duplicate CLI arguments', () => {
  assertCliUsageError(['run', 'https://example.test', '--max-step', '50'], /Unknown option "--max-step"\./);
  assertCliUsageError(
...
test('rejects partial credentials in YAML auth configuration', () => {
  ...
  try {
    writeFileSync(
      path,
      ['version: 1', 'provider: openai', 'model: test-model', 'auth:', '  email: user@example.test'].join('\n'),
    );
...
test('defaults to chromium and maps a YAML browser engine override', () => {
  assert.equal(
    applyConfig(parseArgs(['run', 'https://example.test', '--provider', 'openai', '--model', 'test-model']))
      .browserEngine,
    'chromium',
  );
  ...
  try {
    writeFileSync(
      path,
      ['version: 1', 'url: https://example.test', 'provider: openai', 'model: test-model', 'browser: firefox'].join(
        '\n',
      ),
    );
