...
test('generated login helpers are typed and duplicate flow titles are unique', () => {
  ...
  const auth = generateSpecBundle([{ title: 'Login', name: 'Login', entries: [] }], {
    url: 'https://example.test',
    username: 'tester',
    password: 'secret',
  ...
  assert.match(
    auth.content,
    /findLoginField\(root: Page \| Locator, \.\.\.patterns: RegExp\[\]\): Promise<Locator \| null>/,
  );
  ...
  const credentials = generateSpecBundle([{ title: 'Login', name: 'Login', entries: [] }], {
    url: 'https://example.test',
    username: 'tester',
    password: 'secret',
...
test('generated credential sidecar is immediately usable and owner-readable only', () => {
  ...
  try {
    ...
    assert.equal(
      readFileSync(output.credentialsPath, 'utf8'),
      '{\n  "username": "tester",\n  "password": "secret"\n}\n',
    );
        ...
        entries: [
          {
            ...
            toolCall: {
              name: 'doubleClick',
              input: { locator: 'frame=iframe[title="Payment"] >> role=button[name="Pay"]' },
            },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'dragAndDrop', input: { source: '#source', target: '#drop' } },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'download', input: { locator: '#download' } },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'select', input: { locator: '#tags', value: ['one', 'three'] } },
            network: [],
            console: [],
          },
        ],
...
test('generated JavaScript escapes line terminators and regex locators safely', () => {
  ...
  assert.throws(() => toLocatorExpression('text=/[/i'), /Invalid text locator regular expression/);
});
...
test('codegen validates action inputs and never interpolates handleDialog behavior', () => {
  ...
  assert.throws(
    () =>
      generateSpec(
        [{ name: 'Invalid dialog', entries: [baseEntry('handleDialog', { behavior: 'accept(); hacked()' })] }],
        { url: 'https://example.test' },
      ),
  ...
  assert.throws(
    () =>
      generateSpec([{ name: 'Invalid fill', entries: [baseEntry('fill', { locator: '#name', value: 42 })] }], {
        url: 'https://example.test',
      }),
    /Cannot generate fill: .*\.value must be a string/,
  );

  const spec = generateSpec([{ name: 'Safe dialog', entries: [baseEntry('handleDialog', { behavior: 'dismiss' })] }], {
    url: 'https://example.test',
  });
...
test('codegen rejects malformed expectations before interpolating them', () => {
        ...
        [
          {
            name: 'Invalid count',
            entries: [
              baseEntry({ status: 'met', assertion: 'count', locator: '#items', expectedCount: '0); hacked()' }),
            ],
          },
        ],
        { url: 'https://example.test' },
  ...
  assert.throws(
    () =>
      generateSpec(
        [{ name: 'Invalid assertion', entries: [baseEntry({ status: 'met', assertion: 'count; hacked()' })] }],
        { url: 'https://example.test' },
      ),
        ...
        entries: [
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'click', input: { locator: 'role=button[name="Continue"]' } },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'switchTab', input: { tabId: 'tab-1' } },
            network: [],
            console: [],
          },
        ],
        ...
        entries: [
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'setOffline', input: { offline: true } },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'setOffline', input: { offline: false } },
            network: [],
            console: [],
          },
        ],
        ...
        entries: [
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'click', input: { locator: 'role=button[name="Checkout"]' } },
            network: [],
            console: [],
          },
          {
            ...
            flowIndex: 0,
            timestamp: '2026-01-01T00:00:00.000Z',
            toolCall: { name: 'openInNewTab', input: {} },
            network: [],
            console: [],
          },
        ],
...
test('a flow without a device preset still uses the plain ambient page fixture', () => {
  const spec = generateSpec([{ name: 'Desktop checkout', entries: [] }], { url: 'https://example.test' });
